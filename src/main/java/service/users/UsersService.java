...
public class UsersService {
    ...
    public boolean joinUser(Users user) {
        ...
        try {
            ...
            Users userToInsert = Users.builder()
                                   .userId(user.getUserId())
                                   .username(user.getUsername())
                                   .password(hashedPassword) // 해시된 비밀번호 사용
                                   .email(user.getEmail())
                                   .ROLE(user.getROLE() != null ? user.getROLE() : "USER")
                                   .isEmailVerified(false) // 초기 가입 시 미인증 상태
    ...
    public Optional<Users> loginUser(String userId, String password) {
		...
		try (Connection conn = ConnectionPoolHelper.getConnection()) {
			...
			if (userOpt.isPresent()) {
	            ...
	            // 🚨 보안: Bcrypt를 사용하여 평문 비밀번호와 해시된 비밀번호를 안전하게 비교
    ...
    /**
     * 3. ID 중복 확인 (비동기 처리용)
     * 💡 개선: DAO의 isUserIdExists(COUNT(*)) 메서드를 사용하여 효율적으로 중복 확인
     */
    ...
    /**
     * 6. 이메일 존재 여부 확인 (비동기 및 회원가입 중복 체크용)
     * 💡 새로 추가된 기능
     */
